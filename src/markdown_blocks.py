...
def markdown_to_html_nodes(markdown_file):
...
def quote_to_node(block):
    ...
    for line in lines:
        if not line.startswith(">"):
            raise ValueError("Incorrect quote syntax.")
...
def block_to_html_node(block):
    ...
    if block_type == block_type_olist:
        return ol_list_to_node(block)
    if block_type == block_type_ulist:
        return ul_list_to_node(block)
